...
- Sync source digests now ignore root `.in_use/` runtime markers only for observed installed
  plugins under the active Claude profile's cache, preventing marker churn from invalidating
  conversion plans while keeping configured local sources and unsafe paths fail-closed.
